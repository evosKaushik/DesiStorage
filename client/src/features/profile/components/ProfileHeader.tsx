...
export function ProfileHeader() {
  ...
  const { openPicker, files } = useFileSelectorFromDisk({
    isSelectMultipleFiles: false,
    onlyImagePreview: true,
  });

  const preview = files[0]?.preview;

  // Todo: Implement API call to update user avatar on the server and then update the user in the store with the new avatar URL.
  useEffect(() => {
    if (!user || !preview) return;

    if (user.avatar === preview) return;

    updateUser({ avatar: preview });
  }, [preview, user, updateUser]);
    ...
    <>
      ...
      <div className="flex flex-col gap-6 md:flex-row md:items-center">
        <div className="relative">
          <Avatar className="h-20 w-20 rounded-full border-2 border-border/60 overflow-hidden">
            <AvatarImage

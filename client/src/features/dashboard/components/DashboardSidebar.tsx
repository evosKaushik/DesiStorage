...
export function DashboardSidebar() {
  ...
  return (
    ...
    <>
      ...
      <aside
        className={
          "flex shrink-0 border-t border-border backdrop-blur-lg  sm:hidden absolute z-99 bottom-0 w-full right-0 left-0 h-12 bg-background/40 shadow-2xl shadow-accent-foreground  px-2"
        }
      >
        <nav className="flex gap-0.5  w-full justify-center items-center h-full overflow-x-auto">
          {NAV.map((item) => {
            const active = pathname === "/dashboard" && activeTab === item.id;
            return (
              <Link
                ...
                href={`/dashboard?tab=${item.id}`}
                className={cn(
                  "group flex items-center gap-2 xs:gap-3 rounded-lg px-2 xs:px-3 py-2 text-sm font-medium transition-colors",
                  active
                    ? "bg-primary/10 text-primary"
                    : "text-muted-foreground hover:bg-accent hover:text-foreground",
                  collapsed && "justify-center px-2",
